...
from const import TEST_PLAN_PROPERTY_FILE_NAME, INFRA_PROPERTY_FILE_NAME, LOG_FILE_NAME, DB_META_DATA, \
    PRODUCT_STORAGE_DIR_NAME, MB_DB, DEFAULT_DB_USERNAME, DIST_POM_PATH, NS
...
def get_product_name():
    ...
    product_name = product_name.replace('-launcher', '')
    ...
    for database in db_names:
        if database == MB_DB:
            if db_engine.upper() == 'SQLSERVER-SE':
                ...
                scriptPath = script_path / 'mysql-mb.sql'
                # drop database
                run_mysql_commands('DROP DATABASE IF EXISTS {0};'.format(database))
                # create database
...
def start_broker(broker_home_path):
    """Start ballerina message broker.
    """
    if sys.platform.startswith('win'):
        subprocess.call(broker_home_path + '/broker', shell=True)
    else:
        subprocess.call(['chmod', '+x', broker_home_path + '/broker'])
        subprocess.call(broker_home_path + '/broker', shell=True)
...
def main():
    try:
        ...
        dist_downl_url = get_product_dist_artifact_path(product_dist_download_api) + get_product_dist_rel_path(
            product_dist_download_api)

        # product download path and file name constructing
        product_download_dir = Path(workspace + "/" + PRODUCT_STORAGE_DIR_NAME)
        if Path.exists(product_download_dir):
            shutil.rmtree(product_download_dir, onerror=on_rm_error)
        ...
        # populate databases
        script_path = Path(workspace + "/" + PRODUCT_STORAGE_DIR_NAME + "/message-" + product_name + "/" + 'dbscripts')
        ...
        start_broker(workspace + "/" + PRODUCT_STORAGE_DIR_NAME + "/message-" + product_name + "/bin")
    except Exception as e:
